...
- 新增 `model_registrar` 能力并实现 `model.register`，把目录里的 **`context_length`** 申报给宿主
  （实测上游 `/provider/v1/models` 的 82 条模型都带 `context_length`）。CPA 自己的模型目录
  只收录内置厂商（claude / gemini / codex / kimi 等），插件模型不会被自动补全 ——
  这就是"同一个模型，**直接加到 CPA 时规格正确、换成插件后规格不对**"的原因。
- 「获取上游模型」刷新时，已保存映射里**空着的**规格会自动补上并存盘（触发宿主重新注册），
  不需要手动填；已填过的不覆盖。
- 同时注册**「客户端名」与「上游原名」两个条目**：宿主按客户端请求的模型名找凭据，
  只注册一个名字时，另一种写法会报 `no auth available`（503）。
- 目录里的 `name` 也不再丢弃（面板显示上游给的名字而不是原始 id）。
